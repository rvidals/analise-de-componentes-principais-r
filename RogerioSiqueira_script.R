...
pacman::p_load(tidyverse, # pacote para manipulação de dados
               ...
               utils, # pacote base
               readxl, # pacote para leitura de arquivos excel
               MVN, # pacote para teste de normalidade
               ggcorrplot, # pacote para plotagem de gráficos
               factoextra, # pacote para análise de componentes principais  
               cowplot # pacote para plotagem de gráficos
)
...
TMED <- import(url,  # Nome do arquivo que será importado.
...
TMED %>%
  export("Dados/TMED.xlsx")  # Exporta a base de dados para um arquivo excel.
...
#------------------------------------------------------------------------------#
### PCA com os dados de temperatura média do Sul do Brasil (SUB).

# Selecionando os dados do SEB e transformando em data frame:

SUB <- TMED_mensal |> 
  dplyr::filter(UF %in% c("PR", "SC", "RS")) |> 
  data.frame()
View(SUB)

# Definindo os nomes das linhas:
rownames(SUB) <- paste(SUB$UF, 
                       SUB$Nome.da.Estação, 
                       sep="_") 
...
gpca5 <- 
  ...
  factoextra::fviz_pca_ind(geom = "point",    # Geometria a ser usada no gráfico.
                           habillage = SUB$UF)            # Especificar uma variável qualitativa a ser usada para colorir os indivíduos por grupos.
gpca5
...
gpca9 <- 
  ...
  factoextra::fviz_pca_biplot(geom = "point",   # Geometria a ser usada no gráfico.
                              ...
                              col.ind = SUB$UF,             # Define uma variável categórica a ser usada para colorir indivíduos por grupos.
                              show.legend = F,              # Removendo as letras da legenda.
                              legend.title = "UF",          # Alterando o título da legenda. 
...
gpca10 <- 
  ...
  factoextra::fviz_pca_biplot(geom.ind = "point",  
                              fill.ind = SUB$UF,         # Cor do preenchimento por grupos. 
...
ggplot2::ggsave( 
  plot = gpca10,               # Gráfico criado anteriormente.
  filename = "PCA_biplot.png",   # Nome e extensão do arquivo a ser salvo.
...
ggplot2::ggsave( 
  ...
  filename = "PCA_SUB.png",   # Nome e extensão do arquivo a ser salvo.
  device = "png",             # Formato do arquivo.
  dpi = 600,                  # Resolução do arquivo.
  width = 7000,               # Largura do gráfico. 
  height = 4000,              # Altura do gráfico.
